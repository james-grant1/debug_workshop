...
! equation 2E17 * Erfc(X) = 1 by interval bisection. Note two answers differ!
  ...
  ! Uses an asymptotic expansion for the error function (suitable for x >> 1)
  ! Erf(X) = 1 - (Exp(- X^2) / (Sqrt(Pi) * X))*(1 - 1/(2 * X^2))
  If (X1 > 3.0) Then
    YLower = 2.0E17 * (Exp(- X1 * X1) / (Sqrt(Pi) * X1)) * &
              (1.0 - 1.0/(2.0 * X1 * X1)) - 1.0
  Else
    YLower = 2.0E17 * (1.0 - Erf(X1)) - 1.0
  End If
  
  If (X2 > 3.0) Then
    YUpper = 2.0E17 * (Exp(- X2 * X2) / (Sqrt(Pi) * X2)) * &
              (1.0 - 1.0/(2.0 * X2 * X2)) - 1.0
  Else
    YUpper = 2.0E17 * (1.0 - Erf(X2)) - 1.0
  End If
  Write(6,*) 'Y ', YLower,' at X ',X1
  ...
  XUpper = X2
  If (YUpper * YLower < 0.0) Then
    Write(6,*) 'Solution of approx Erf eq exists between X = ',XLower,' and ',XUpper
    Call Eqn_ApproxBisection(XUpper, XLower, YLower, YUpper, nIter)
    Write(6,*) 'Solution of approx Erf eq exists between X = ',XLower,' and ',XUpper
  Else If (YUpper * YLower > 0.0) Then
    Write(6,*) 'No solution of approx Erf eq exists between X = ',XLower,' and ',XUpper
    Stop
  Else If (YUpper == 0.0) Then
    Write(6,*) 'Solution of approx Erf eq is X = ',XUpper
  Else If (YLower == 0.0) Then
    Write(6,*) 'Solution of approx Erf eq is X = ',XLower
  End If
...
Subroutine Eqn_ApproxBisection(XUpper, XLower, EqnLower, EqnUpper, nIter)
! Solves error function equation by interval bisection but using an asymptotic 
! expansion for Erf(x) when x >> 1
!
  ...
  Do i = 1,nIter
    X = (XUpper + XLower) / 2.0
    If (X > 3.0) Then
      EqnMid = 2.0E17 * (Exp(- X * X) / (Sqrt(Pi) * X)) * &
                (1.0 - 1.0/(2.0 * X * X)) - 1.0
...
End Subroutine Eqn_ApproxBisection
